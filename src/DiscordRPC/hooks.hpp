...
namespace hooks {
	...
	void hk_baseplayer_ClientInput(BasePlayer* baseplayer, input_state* state/*, Projectile* prj*/) {
		...
		if (!serverrpc_projecshoot) {
			auto method_serverrpc_projecshoot = *reinterpret_cast<uintptr_t*>(mem::game_assembly_base + 50098416);//Method$BaseEntity.ServerRPC<ProjectileShoot>()
		  
			/*AllocConsole();
			FILE* P = 0;
			freopen_s(&P, "con", "w", stdout);
			printf("0x%llx\n", method_serverrpc_projecshoot);*/
			if (method_serverrpc_projecshoot && method_serverrpc_projecshoot != 0xc00312e1) {
				 
				serverrpc_projecshoot = (uintptr_t*)((**(uintptr_t**)(method_serverrpc_projecshoot + 0x38)) + 0x8);
		...
		if (baseplayer) {
			...
			if (vars::visuals::hand_chams) {
				auto oxunem = il2cpp::init_class(("Graphics"), ("ConVar"));
					...
					if (can_manipulate(baseplayer,  mm_eye) && vars::desyncTime > 0.40f)//test
					{
						if (nextshot < fixed_time && (held->get_time_since_deploy() > held->get_deploy_delay() && mag_ammo > 0)) {
							...
							if (vars::weapon::autoshoot)
							{
								...
								typedef void(__fastcall* oxunem)(BaseProjectile*, rust::classes::Signal, rust::classes::string);
								((oxunem)(mem::game_assembly_base + O::BaseEntity::SendSignalBroadcast))(held, rust::classes::Signal::Attack, _(L""));
							}
