...
public:
	...
	inline float Distance(const vector2& Dst) {
		return my_sqrt(powFFFFFFFFFFFFFFFFFFFFFF(this->x - Dst.x, 2) + powFFFFFFFFFFFFFFFFFFFFFF(this->y - Dst.y, 2));
	}
...
public:
	...
	float get_3d_dist(const Vector& Dst) {
	...
	static Vector move_towards(Vector current, Vector target, float maxDistanceDelta)
	{
		...
		return Vector(current.x + toVector_x / dist * maxDistanceDelta,
...
namespace Math {
	...
	inline Vector rotate_point(Vector center, Vector origin, float angle) {
		float num = angle * 0.0174532924f;
		float num2 = -m_sin(num);
		float num3 = m_cos(num);
		origin.x -= center.x;
		origin.z -= center.z;
		...
		num5 += center.z;
		return Vector(num6, origin.y, num5);
	}
}
